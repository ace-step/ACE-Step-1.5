...
def autolabel_single_with_music_flamingo(
    dit_handler,
    llm_handler,
    sample_idx: int,
    builder_state: Optional[DatasetBuilder],
    skip_metas: bool = False,
) -> Tuple[Any, str, DatasetBuilder]:
    """Autolabel a single selected sample using the Music-Flamingo online backend.

    Returns:
        Tuple of (table_update, status_update, builder_state)
    """
    if builder_state is None or not builder_state.samples:
        return gr.update(value=[]), "❌ No dataset loaded", builder_state

    if sample_idx is None:
        return gr.update(value=builder_state.get_samples_dataframe_data()), "❌ No sample selected", builder_state

    idx = int(sample_idx)
    if idx < 0 or idx >= len(builder_state.samples):
        return gr.update(value=builder_state.get_samples_dataframe_data()), "❌ Invalid sample index", builder_state

    # Force the Music-Flamingo provider for this action.
    _, status = builder_state.label_sample(
        idx,
        dit_handler,
        llm_handler,
        format_lyrics=False,
        ...
        provider_override="music-flamingo",
    )

    table_data = builder_state.get_samples_dataframe_data()
    return gr.update(value=table_data), status, builder_state
